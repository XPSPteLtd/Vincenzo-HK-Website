    ...
    <div className="fixed inset-0 z-[140] flex items-center justify-center p-0 md:p-6">
      ...
      <div className="relative bg-[#F7F7F7] w-full max-w-md h-full md:h-[85vh] md:max-h-[750px] shadow-2xl animate-fade-in-up flex flex-col overflow-hidden text-charcoal md:rounded-3xl mt-[57px] mb-[64px] md:mt-0 md:mb-0">
          ...
          {!selectedPlatform ? (
            ...
            <div className="p-8 flex flex-col h-full animate-fade-in">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-6 border-t border-gray-200">
                  <button className="w-full p-6 rounded-2xl border-2 border-dashed border-gray-200 hover:border-gold/30 hover:bg-white transition-all flex items-center gap-4 group">
                    ...
                    <div className="text-left">
                      <h4 className="font-bold text-lg">{lang !== 'en' ? '餐飲外送及活動' : 'Catering & Events'}</h4>
                      <p className="text-[10px] text-gray-400 uppercase tracking-widest">{lang !== 'en' ? '大額訂單專人服務' : 'Direct Large Order Concierge'}</p>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div className="mt-auto pt-8 text-center">
                <p className="text-[9px] text-gray-400 uppercase tracking-[0.3em]">{lang !== 'en' ? '招牌當代拿坡里手工麵團' : 'Signature contemporary neapolitan dough'}</p>
              </div>
            </div>
          ) : (
            <div className="p-6 space-y-6 animate-fade-in">
              ...
              >
                <div className="relative z-10">
                  <h3 className="font-bold text-xl mb-1">{lang !== 'en' ? '高達 8 折優惠' : 'UP TO 20% OFF'}</h3>
                  <p className="text-xs opacity-90">{lang !== 'en' ? '優惠碼：' : 'Use Code:'} {currentPlatform?.promoCode}</p>
                </div>
        ...
        <div className="bg-charcoal px-6 py-4 flex items-center justify-between text-white shrink-0">
           ...
           <span className="text-[9px] text-gray-600 uppercase tracking-widest font-bold">{lang !== 'en' ? '拿坡里薄餅專門店' : 'Luxury Pizzeria Management'}</span>
        </div>
      </div>
    </div>
  );
